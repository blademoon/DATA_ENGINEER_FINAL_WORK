...
insert into 
	DEMIPT.ANBO_STG_BANK_ACCOUNTS( ACCOUNT, VALID_TO, CLIENT, CREATE_DT, UPDATE_DT )
select
...
where 
	COALESCE(UPDATE_DT,CREATE_DT) > (
		...
		where 
			DBNAME = 'DEMIPT' 
		and 
			TABLENAME = 'ANBO_DWH_DIM_ACCOUNTS_HIST'
	);


insert into 
	DEMIPT.ANBO_STG_BANK_CARDS( CARD_NUM, ACCOUNT, CREATE_DT, UPDATE_DT )
select 
	CARD_NUM, 
	ACCOUNT, 
	CREATE_DT, 
...
where 
	COALESCE(UPDATE_DT,CREATE_DT) > (
		...
		where 
			DBNAME = 'DEMIPT' 
		and 
			TABLENAME = 'ANBO_DWH_DIM_CARDS_HIST'
		);


insert into 
	DEMIPT.ANBO_STG_BANK_CLIENTS( CLIENT_ID, LAST_NAME, FIRST_NAME, PATRONYMIC, DATE_OF_BIRTH, PASSPORT_NUM, PASSPORT_VALID_TO, PHONE, CREATE_DT, UPDATE_DT )
select 
	CLIENT_ID, 
	LAST_NAME, 
	FIRST_NAME, 
...
where 
	COALESCE(UPDATE_DT,CREATE_DT) > ( 
		...
		where 
			DBNAME = 'DEMIPT' 
		and 
			TABLENAME = 'ANBO_DWH_DIM_CLIENTS_HIST'
		);


INSERT INTO 
	DEMIPT.ANBO_STG_TERMINALS ( TERMINAL_ID, TERMINAL_TYPE, TERMINAL_CITY, TERMINAL_ADDRESS, CREATE_DT, UPDATE_DT )
SELECT
	t1.TERMINAL_ID,
	t1.TERMINAL_TYPE,
	t1.TERMINAL_CITY,
	t1.TERMINAL_ADDRESS,
	MIN(t2.EFFECTIVE_FROM) OVER (PARTITION BY t2.TERMINAL_ID ORDER BY t2.EFFECTIVE_FROM) as CREATE_DT, TO_DATE(t1.UPLOAD_DT,'DD.MM.YYYY')
FROM 
	ANBO_STG_TERMINALS_DRFT t1
LEFT JOIN 
	ANBO_DWH_DIM_TERMINALS_HIST t2
ON 
	t1.terminal_id = t2.TERMINAL_ID
WHERE
	t2.EFFECTIVE_TO = TO_DATE('31.12.2021','DD.MM.YYYY')
AND
...
INSERT INTO 
	DEMIPT.ANBO_DWH_FACT_TRANSACTIONS( TRANS_ID, TRANS_DATE, CARD_NUM, OPER_TYPE, AMT, OPER_RESULT, TERMINAL)
SELECT
	TRANSACTION_ID,
...
INSERT INTO 
	ANBO_DWH_FACT_PSSPRT_BLCKLST( PASSPORT_NUM, ENTRY_DT )
SELECT
	PASSPORT_NUM,
	TO_DATE(ENTRY_DT, 'YYYY-MM-DD')
FROM 
	ANBO_STG_PSSPRT_BLCKLST
WHERE 
	TO_DATE(ENTRY_DT, 'YYYY-MM-DD') > (
		SELECT 
			LAST_UPDATE 
		...
		WHERE 
			DBNAME = 'DEMIPT' 
		AND 
			TABLENAME = 'ANBO_DWH_FACT_PSSPRT_BLCKLST'
	);



INSERT INTO 
	DEMIPT.ANBO_DWH_DIM_TERMINALS_HIST( TERMINAL_ID, TERMINAL_TYPE, TERMINAL_CITY, TERMINAL_ADDRESS, EFFECTIVE_FROM, EFFECTIVE_TO, DELETED_FLG )
SELECT
	TERMINAL_ID,
	TERMINAL_TYPE, 
	TERMINAL_CITY, 
	...
	'N'
FROM 
	DEMIPT.ANBO_STG_TERMINALS;

MERGE INTO 
	DEMIPT.ANBO_DWH_DIM_TERMINALS_HIST tgt
USING 
	DEMIPT.ANBO_STG_TERMINALS src
ON 
	( tgt.TERMINAL_ID = src.TERMINAL_ID and tgt.EFFECTIVE_FROM < COALESCE( src.UPDATE_DT, src.CREATE_DT ) )
WHEN 
	matched THEN UPDATE SET tgt.EFFECTIVE_TO = COALESCE( src.UPDATE_DT, src.CREATE_DT ) - interval '1' second;
WHERE 
	tgt.EFFECTIVE_TO = TO_DATE( '2999-12-31', 'YYYY-MM-DD' )



INSERT INTO 
	DEMIPT.ANBO_DWH_DIM_ACCOUNTS_HIST( ACCOUNT_NUM, VALID_TO, CLIENT, EFFECTIVE_FROM, EFFECTIVE_TO, DELETED_FLG )
SELECT
	ACCOUNT,
	VALID_TO, CLIENT,
	COALESCE( UPDATE_DT, CREATE_DT ),
...
MERGE INTO 
	DEMIPT.ANBO_DWH_DIM_ACCOUNTS_HIST tgt
USING 
	DEMIPT.ANBO_STG_BANK_ACCOUNTS src
...
INSERT INTO 
	DEMIPT.ANBO_DWH_DIM_CARDS_HIST( CARD_NUM, ACCOUNT_NUM, EFFECTIVE_FROM, EFFECTIVE_TO, DELETED_FLG )
SELECT
	CARD_NUM,
	ACCOUNT,
	COALESCE( UPDATE_DT, CREATE_DT ),
...
MERGE INTO 
	DEMIPT.ANBO_DWH_DIM_CARDS_HIST tgt
USING 
	DEMIPT.ANBO_STG_BANK_CARDS src
ON ( tgt.CARD_NUM = src.CARD_NUM and tgt.EFFECTIVE_FROM < COALESCE( src.UPDATE_DT, src.CREATE_DT ) )
WHEN 
	matched THEN UPDATE SET tgt.EFFECTIVE_TO = COALESCE( src.UPDATE_DT, src.CREATE_DT ) - interval '1' second;
WHERE 
	tgt.EFFECTIVE_TO = TO_DATE( '2999-12-31', 'YYYY-MM-DD' );
...
INSERT INTO 
	DEMIPT.ANBO_DWH_DIM_CLIENTS_HIST( CLIENT_ID, LAST_NAME, FIRST_NAME, PATRONYMIC, DATE_OF_BIRTH, PASSPORT_NUM, PASSPORT_VALID_TO, PHONE, EFFECTIVE_FROM, EFFECTIVE_TO, DELETED_FLG )
SELECT
	CLIENT_ID,
	LAST_NAME, 
	FIRST_NAME, 
...
MERGE INTO 
	DEMIPT.ANBO_DWH_DIM_CLIENTS_HIST tgt
USING 
	DEMIPT.ANBO_STG_BANK_CLIENTS src
...
INSERT INTO 
	DEMIPT.ANBO_STG_TERMINALS_DEL ( TERMINAL_ID )
select 
	TERMINAL_ID 
FROM 
...
INSERT INTO 
	DEMIPT.ANBO_STG_BANK_ACCOUNTS_DEL ( ACCOUNT )
select 
	ACCOUNT 
FROM 
	BANK.ACCOUNTS;
...
INSERT INTO 
	DEMIPT.ANBO_STG_BANK_CARDS_DEL ( CARD_NUM )
select 
	CARD_NUM 
FROM 
	BANK.CARDS;
...
INSERT INTO 
	DEMIPT.ANBO_STG_BANK_CLIENTS_DEL ( CLIENT_ID )
select 
	CLIENT_ID 
FROM 
	BANK.CLIENTS;
...
SELECT
	...
	'Y'
FROM 
	ANBO_DWH_DIM_TERMINALS_HIST t
LEFT JOIN 
	ANBO_STG_TERMINALS_DEL s
ON 
	t.TERMINAL_ID = s.TERMINAL_ID
WHERE 
	s.TERMINAL_ID IS NULL
AND 
	EFFECTIVE_TO = TO_DATE( '2999-12-31', 'YYYY-MM-DD' )
AND 
	deleted_flg = 'N';
...
SELECT
	...
	'Y'
FROM 
	ANBO_DWH_DIM_ACCOUNTS_HIST t
LEFT JOIN 
	ANBO_STG_BANK_ACCOUNTS_DEL s
ON 
	t.ACCOUNT_NUM = s.ACCOUNT
WHERE 
	s.ACCOUNT IS NULL
AND 
	EFFECTIVE_TO = TO_DATE( '2999-12-31', 'YYYY-MM-DD' )
...
SELECT
	...
	'Y'
FROM 
	ANBO_DWH_DIM_CARDS_HIST t
LEFT JOIN 
	ANBO_STG_BANK_CARDS_DEL s
ON 
	t.CARD_NUM = s.CARD_NUM
WHERE 
	s.CARD_NUM IS NULL
AND 
	EFFECTIVE_TO = TO_DATE( '2999-12-31', 'YYYY-MM-DD' )
...
SELECT
	...
	'Y'
FROM 
	ANBO_DWH_DIM_CLIENTS_HIST t
LEFT JOIN 
	ANBO_STG_BANK_CLIENTS_DEL s
ON 
...
SET last_update = ( SELECT MAX( TO_DATE(UPLOAD_DT, 'DD.MM.YYYY') )
FROM DEMIPT.ANBO_STG_TERMINALS_DRFT )
WHERE dbname = 'DEMIPT'
AND 
	tablename = 'ANBO_DWH_DIM_TERMINALS_HIST'
AND ( SELECT MAX( TO_DATE(UPLOAD_DT, 'DD.MM.YYYY') )
	FROM 
		DEMIPT.ANBO_STG_TERMINALS_DRFT ) IS NOT NULL;
...
SET
	last_update = (
		...
		AND ( SELECT MAX( COALESCE( UPDATE_DT, CREATE_DT ) )
				FROM 
					DEMIPT.ANBO_STG_BANK_CLIENTS ) IS NOT NULL;
